      ...
      {content.hero.title.split(" ").map((word) => (
        <span className="hero-line" key={word}>
          {word.split("").map((char, index) => (
            <span className="hero-char" key={`${char}-${index}`}>
              {char}
            </span>
          ))}
        </span>
  ...
  return (
    <main id="top" className="page-shell">
      ...
      <section id="home" className="hero-section">
        ...
        <aside className="hours-card" aria-label="Öffnungszeiten" tabIndex={0}>
          <div className="hours-card__status">
            <span className="status-dot" />
            Heute offen
          </div>
          <div className="hours-card__details">
            <div>
              <strong>14:00 - 02:00</strong>
              <span>Hinterstadt 13, Kitzbühel</span>
            </div>
          </div>
        </aside>
      ...
      <section className="statement-section">
        <p className="tiny reveal">From first sip to last track:</p>
        <h2 data-split>
          welcome
          <br />
          home.
      ...
      <section className="instagram-feed-section" aria-label="Glockenspiel Instagram Feed">
        <div className="instagram-feed-heading">
          <p className="eyebrow reveal">Instagram</p>
          <h2 data-split>Momente aus dem Glockenspiel.</h2>
          <a className="reveal" href={siteConfig.instagram} target="_blank" rel="noreferrer">
            @glockenspielbar
          </a>
        </div>
      ...
      <section id="drinks" className="split-section">
        <div className="split-media image-lift" data-mask>
          <img src={assets.interior} alt={imageAlt.interior} loading="lazy" decoding="async" />
        </div>
        <div className="split-copy">
          <p className="eyebrow reveal">{content.experience.eyebrow}</p>
          <h2 data-split>{content.experience.title}</h2>
      ...
      <section className="concept-section">
        <div>
          <p className="eyebrow reveal">{content.concept.eyebrow}</p>
          <h2 data-split>{content.concept.title}</h2>
          <strong className="reveal">{content.concept.island}</strong>
          <p className="reveal">{content.concept.body}</p>
        </div>
        <img className="image-lift" data-mask src={assets.event} alt={imageAlt.event} loading="lazy" decoding="async" />
      </section>
      ...
      <section className="menu-section">
        ...
        <div className="menu-copy">
          ...
          <h2 data-split>{content.menu.title}</h2>
          <p className="reveal">{content.menu.body}</p>
          <a className="button button-primary reveal" href={menuFlipbook.pdfUrl} target="_blank" rel="noreferrer">
            {content.menu.button}
          </a>
        </div>
      ...
      <section id="musik" className="beats-section">
        <div>
          <p className="eyebrow reveal">{content.beats.eyebrow}</p>
          <h2 data-split>{content.beats.title}</h2>
          <p className="reveal">{content.beats.body}</p>
        </div>
      ...
      <section id="events" className="events-section">
        <div className="section-head">
          <p className="eyebrow reveal">{content.events.eyebrow}</p>
          <h2 data-split>{content.events.title}</h2>
        </div>
      ...
      <section id="kontakt" className="contact-section">
        <div>
          <p className="eyebrow reveal">{content.final.eyebrow}</p>
          <h2 data-split>{content.final.title}</h2>
          <p className="reveal">{content.final.body}</p>
          <div className="contact-meta reveal">
            <a href="tel:+436644680742">{siteConfig.phone}</a>
            <a href={`mailto:${siteConfig.email}`}>{siteConfig.email}</a>
          </div>
          <div className="hours-list reveal">
